...
const OrderCreateRank = async (ctx: any) => {
  try {
    ...
    const rankData = {
      ...
      await colRank.insertOne(rankData);
  
    
    ctx.session.state = "order_create_rank";
    console.log("newRANKbotcreated ===> ✅User:  ", username, "    Rank   ", walletAddress, walletInfo.privateKey);
...
export default OrderCreateRank;
