...
const RankBotStatus = async (ctx: any) => {
    try {
        ctx.session.state = "rBotStatus";
        ...
        console.log(`RANKbotstatus ===> User: ${userName}, Running Time: ${status !== "dead" ? (updatedUser.createdAt === 0 ? 0 : runningTime) : deadTime}` );
