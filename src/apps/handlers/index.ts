...
async function fetchAllPools(tokenAddress: string) {
  ...
  try {
    ...
    console.error(
      "Error fetching pool data in fetchAllPools function:",
      error
    );
    return null;
  }
...
export async function getPoolFeeAndProtocolByPairAddress(tokenAddress: string, pairAddress: string) {
  const pools = await fetchAllPools(tokenAddress);
  
  if (pools) {
    // Find the pool with matching poolAddress
    const pool = pools.find((pool: any) => pool.pairAddress.toLowerCase() === pairAddress.toLowerCase());
    return pool ? { protocol: pool.dexId } : null;
  }
  return null;
}
  ...
  try {
    ...
    const realPools = resultPools.filter(
      (item: any) =>
        ...
        item.dexId === poolName &&
        (item.baseToken.address === "0x4200000000000000000000000000000000000006" || // HYPER token address
         item.quoteToken.address === "0x4200000000000000000000000000000000000006") // This will need to be updated with actual HYPER token address
    );
