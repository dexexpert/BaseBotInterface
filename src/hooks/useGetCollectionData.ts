...
export async function useGetCollectionData(
  ...
  try {
    ...
    const mintStageRes = await getActiveOrNextMintStage({
      collection_address,
    });
    ...
    const { startDate, endDate, isMintInfinite } =
      await getMintStageStartAndEndTime({
        collection_address,
        mint_stage,
      });
